...
void percwrite(char *percfile, int** map, int ncluster, int len) {
  FILE *fp;
  int i, j, clusterid, icluster, maxcluster, prevcluster, colour, npix;
  int clustersize[len*len];
  ...
  prevcluster = len*len+1; // Larger than the largest possible cluster
   ...
   *  bottom-left-hand corner and map[len-1][len-1] is in the
